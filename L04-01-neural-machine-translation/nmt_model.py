...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        # Projection with W_{attProj}
        self.att_projection = nn.Linear(
            in_features=hidden_size*2,  # We receive [h_dec; h_enc]
            out_features=hidden_size,  # Shouldn't this be m? Perhaps m == h?
            bias=False
        )

        # Projection with W_{u}
        self.combined_output_projection = nn.Linear(
            in_features=hidden_size*3,  # check the input size of u_t
            out_features=hidden_size,
            bias=False
        )

        # Projection with W_{vocab}
        self.target_vocab_projection = nn.Linear(
            in_features=hidden_size,  # We input o_t of shape (hx1)
            out_features=len(vocab.tgt),  # We ouput a probab. distribution for all the words in the target vocab
            bias=False
        )
    ...
    def beam_search(self, src_sent: List[str], beam_size: int=5, max_decoding_time_step: int=70) -> List[Hypothesis]:
        ...
        while len(completed_hypotheses) < beam_size and t < max_decoding_time_step:
            ...
            # prev_hyp_ids = top_cand_hyp_pos // len(self.vocab.tgt) ... this way of dividing a tensor will be deprecated
